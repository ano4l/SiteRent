  ...
  async function onSubmit(formData: FormData) {
    ...
    const response = await fetch("/api/ai/website-plan", {
      ...
      body: JSON.stringify({
        ...
        preferredTemplateStyle: formData.get("preferredTemplateStyle") || undefined,
        businessContext: formData.get("businessContext"),
        currentWebsiteContext: formData.get("currentWebsiteContext") || undefined
      })
    });
  ...
  return (
    <section className="rounded-[32px] border border-[#e1d8ca] bg-white p-6 shadow-[0_16px_38px_rgba(17,17,17,0.05)] xl:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-[#ddd5c7] bg-[#fbf8f3] px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-[#7e7165]">
          Server-side Gemini
        </span>
      </div>
      ...
      <form action={onSubmit} className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="grid gap-4">
          <label className="text-sm font-bold text-ink">
            Mode
            <select name="mode" defaultValue="create" className="mt-2 h-11 w-full rounded-2xl border border-[#ddd5c7] bg-[#fbf8f3] px-4 outline-none transition focus:border-[#c97c16] focus:bg-white">
          ...
          <label className="text-sm font-bold text-ink">
            Template style
            <select name="preferredTemplateStyle" defaultValue="" className="mt-2 h-11 w-full rounded-2xl border border-[#ddd5c7] bg-[#fbf8f3] px-4 outline-none transition focus:border-[#c97c16] focus:bg-white">
          ...
          <label className="text-sm font-bold text-ink">
            ...
            <textarea name="currentWebsiteContext" className="mt-2 min-h-32 w-full rounded-2xl border border-[#ddd5c7] bg-[#fbf8f3] px-4 py-3 outline-none transition placeholder:text-[#a69a8c] focus:border-[#c97c16] focus:bg-white" placeholder="Optional notes about the existing website, photos, weak sections, or requested UI changes." />
          </label>
        </div>
        ...
        <div className="grid gap-4">
          <label className="text-sm font-bold text-ink">
            ...
            <textarea
              ...
              required
              minLength={20}
              className="mt-2 min-h-48 w-full rounded-2xl border border-[#ddd5c7] bg-[#fbf8f3] px-4 py-3 outline-none transition placeholder:text-[#a69a8c] focus:border-[#c97c16] focus:bg-white"
      ...
      {plan && (
        <div className="mt-6 grid gap-4 rounded-[28px] border border-[#e8dfd2] bg-[#fbf8f3] p-5 lg:grid-cols-2 xl:p-6">
          <div>
            ...
            <div className="mt-4 rounded-[22px] border border-[#e8dfd2] bg-white p-4 shadow-[0_10px_24px_rgba(17,17,17,0.04)]">
          ...
          <div>
            ...
            <div className="mt-2 grid gap-3">
              {plan.uiChangePlan.slice(0, 4).map((item) => (
                <article key={`${item.area}-${item.change}`} className="rounded-[20px] border border-[#e8dfd2] bg-white p-4 shadow-[0_10px_24px_rgba(17,17,17,0.04)]">
